...
function generateProductData() {
  const categories = {
    ...
    packaging: {
      ...
      collection: 'Premium Packaging',
    },
  };

  const styles = ['Natural', 'Dramatic', 'Winged', 'Cat-Eye'];
  const colors = ['Black', 'Brown', 'Red', 'Pink', 'Gold', 'Nude'];
  const shapes = ['Round', 'Teardrop', 'Wedge'];
  ...
  for (const [catKey, cat] of Object.entries(categories)) {
    for (let i = 0; i < cat.count; i++) {
      ...
      let title = `${cat.type} Product ${i + 1}`;
      if (catKey === 'eyelashes') {
        title = `${styles[i % styles.length]} ${colors[i % colors.length]} Eyelashes`;
      } else if (catKey === 'nails') {
        title = `${colors[i % colors.length]} Nail Set ${i + 1}`;
      } else if (catKey === 'sponges') {
        title = `${shapes[i % shapes.length]} ${colors[i % colors.length]} Sponge`;
      } else if (catKey === 'packaging') {
        title = `${colors[i % colors.length]} Gift Box ${i + 1}`;
      }

      products.push({
        sku: `${catKey.toUpperCase().substring(0, 2)}-${String(i + 1).padStart(3, '0')}`,
        name: title,
        description: `Premium ${cat.type.toLowerCase()} product for luxurious beauty.`,
        category: cat.type,
        ...
        tags: [cat.type.toLowerCase(), 'luxury', 'premium'],
...
async function main() {
  try {
    ...
    if (count > 0) {
      ...
      console.log('To re-import, clear the products table first.\n');
    }
