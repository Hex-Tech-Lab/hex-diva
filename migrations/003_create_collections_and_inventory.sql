-- Collections and Inventory Infrastructure for Hex-Diva Product Import
...
create index if not exists idx_inventory_sync_product_id on public.inventory_sync_log(product_id);
...
create policy if not exists "Collections are readable by everyone"
  on public.collections for select using (true);

create policy if not exists "Product collections are readable by everyone"
  on public.product_collections for select using (true);

create policy if not exists "Product variants are readable by everyone"
  on public.product_variants for select using (true);

create policy if not exists "Search index is readable by everyone"
  on public.product_search_index for select using (true);
